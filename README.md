...
This hook is built to solve the MEV problem for low-liquidity pools - a ready-to-use solution for creator coin pools, niche memecoins or launchpads. It uses a priority fee median to track unusual behavior and make MEV-patterned swaps less profitable.
...
1. __Reads the priority fee__ - Checks the extra fee paid for faster transaction inclusion.
2. __Compares it to the norm__ - Measures it against the recent typical level.
3. __Normal swaps pay 0.1%__ - No extra fee when the bid is near the norm.
4. __Aggressive bids pay more__ - Higher-than-normal priority fees trigger a dynamic surcharge.
5. __The swap executes normally__ - The extra fee goes to the pool.
6. __Updates from meaningful swaps__ - Eligible trades can update the typical priority fee.
7. __Resists manipulation__ - Only eligible pools and meaningful price movements affect the shared reference.

---

## Explore the hook

#### 🔮 [DOCUMENTATION](https://pulse-hook.mintlify.site/introduction) - find much more structured information about the hook here.
...
#### ⚗️ [DEPLOY HOOK](https://pulse-hook.mintlify.site/guides/overview-2) - see how to install, test, evaluate, modify, and deploy the hook.
